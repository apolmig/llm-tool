...
const OutputArea: React.FC<OutputAreaProps> = ({ results }) => {
  ...
  const SummaryCard = ({ model, text, isShortest }: { model: string; text: string; isShortest: boolean }) => {
    ...
    return (
      <div className={`flex flex-col h-full bg-slate-900/50 border rounded-xl overflow-hidden transition-all duration-300 ${isShortest ? 'border-emerald-500/50 shadow-lg shadow-emerald-900/10' : 'border-slate-700'}`}>
        {/* Header for Card */}
          ...
          <div className="flex items-center gap-2">
            ...
            {isShortest && (
                 <div className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-emerald-500/20 border border-emerald-500/30">
                    ...
                    <span className="text-[9px] font-bold text-emerald-300 uppercase tracking-wider">Most Concise</span>
                 </div>
             )}
          </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto custom-scrollbar">
           {isError ? (
              <div className="text-red-400 text-sm font-mono">{text}</div>
           ) : (
  ...
  if (!hasResults) {
    return (
      <div className="flex flex-col h-full p-6 items-center justify-center text-slate-600">
        ...
        <p className="text-sm font-medium">Summaries will appear here</p>
        <p className="text-xs mt-2 text-slate-700">Select multiple models to compare results.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-slate-100 flex items-center gap-2">
            Results
            {isComparison && <span className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded-full">Comparison Mode</span>}
        </h2>
      </div>
